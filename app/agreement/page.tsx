...
export default function AgreementPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/upload-pdf`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
...
const clearSignature = () => {
  ...
  return (
    <>
      <div className="max-w-4xl mx-auto p-4 sm:p-6 bg-white rounded-xl shadow-md mt-6 text-sm sm:text-base overflow-x-auto" ref={pdfRef}>
        <h1 className="text-xl sm:text-2xl font-bold mb-4 text-center">Loan Agreement</h1>

        {agreementData && (
          <>
            <table className="w-full table-auto border mb-6 text-xs sm:text-sm">
            ...
            <div className="overflow-x-auto mb-4">
              ...
              <table className="min-w-full table-auto border text-xs sm:text-sm">
                <thead className="bg-gray-100">
                  <tr>
                    <th className="border px-2 sm:px-4 py-2">Installment</th>
                    <th className="border px-2 sm:px-4 py-2">Repayment Date</th>
                    <th className="border px-2 sm:px-4 py-2">Principal</th>
                    <th className="border px-2 sm:px-4 py-2">Interest</th>
                    <th className="border px-2 sm:px-4 py-2">Total</th>
                  </tr>
                </thead>
                <tbody>
  {agreementData?.repaymentSchedules?.length > 0 ? (
    agreementData.repaymentSchedules.map((repayment: any, index: number) => (
      <tr key={index}>
        ...
        <td className="border px-2 sm:px-4 py-2">{repayment?.repaymentdate}</td>
        <td className="border px-2 sm:px-4 py-2">₹{repayment?.principalamount}</td>
        <td className="border px-2 sm:px-4 py-2">₹{repayment?.interest}</td>
        <td className="border px-2 sm:px-4 py-2">₹{repayment?.repayableamount}</td>
      </tr>
    ))
  ) : (
    <tr>
